...
var (
	reconcilers = map[string]Reconciler{
		"specializer": &specializerreconciler.SpecializerReconciler{},
		"edgestatus":  &controllers.NfDeployReconciler{},
	}
)
...
type Reconciler interface {
	...
	SetupWithManager(context.Context, ctrl.Manager, interface{}) error
}
...
func run(ctx context.Context) error {
	...
	for name, reconciler := range reconcilers {
		...
		if err = reconciler.SetupWithManager(ctx, mgr, config.SpecializerControllerConfig{
