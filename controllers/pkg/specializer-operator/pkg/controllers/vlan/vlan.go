...
func Setup(ctx context.Context, mgr ctrl.Manager, cfg config.SpecializerControllerConfig) specializerreconciler.Config {
	r := &function.FnR{ClientProxy: vlan.New(
		ctx, clientproxy.Config{Address: cfg.Address},
	)}

	return specializerreconciler.Config{
