...
import (
	...
	"github.com/ssharmasandeep/nephio/controllers/pkg/specializer-operator/pkg/controllers/ipam"
	"github.com/ssharmasandeep/nephio/controllers/pkg/specializer-operator/pkg/controllers/vlan"
...
func (r *SpecializerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, i interface{}) error {
	...
	for _, setup := range []func(context.Context, ctrl.Manager, config.SpecializerControllerConfig) Config{
		vlan.Setup,
		ipam.Setup,
	} {
		cfg := setup(ctx, mgr, i.(config.SpecializerControllerConfig))
		//ge := make(chan event.GenericEvent)
		r.Client = mgr.GetClient()
		r.For = cfg.For
		r.porchClient = cfg.PorchClient
		r.krmfn = cfg.KRMfunction
	}
